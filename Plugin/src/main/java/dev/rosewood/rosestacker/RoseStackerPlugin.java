 ...
 * @author Esophose
 */
public class RoseStackerPlugin extends RoseStacker {

    /**
     * The running instance of RoseStacker on the server
     */
    private static RoseStackerPlugin instance;

    @ApiStatus.Internal
    public static RoseStackerPlugin getInstance() {
        return instance;
    }

    public RoseStackerPlugin() {
        ...
        try {
            ReflectionUtils.getMethodByName(RoseStackerAPI.class, "initialize", RoseStacker.class).invoke(null, this);
    ...
    public <T extends ManagerLogic> T getManagerInternal(Class<T> managerClass) {
        if (managerClass == StackManagerLogic.class)
            return (T) this.getManager(StackManager.class);
        if (managerClass == StackSettingManagerLogic.class)
            return (T) this.getManager(StackSettingManager.class);
        return null;
    }
