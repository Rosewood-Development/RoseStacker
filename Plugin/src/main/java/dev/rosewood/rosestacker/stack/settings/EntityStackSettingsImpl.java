...
public class EntityStackSettingsImpl extends BaseStackSettings implements EntityStackSettings {
    ...
    public EntityStackSettingsImpl(CommentedFileConfiguration settingsFileConfiguration, JsonObject jsonObject, EntityType entityType) {
        ...
        List<StackConditions.StackCondition<?>> stackConditions = StackConditions.getEligibleConditions(this::isEntity);
        ...
        switch (this.entityType) {
            case CHICKEN -> this.putSetting("multiply-egg-drops-by-stack-size", true);
            case SHEEP -> {
                this.putSetting("shear-all-sheep-in-stack", true);
                this.putSetting("percentage-of-wool-to-regrow-per-grass-eaten", 25);
            }
            case SLIME, MAGMA_CUBE -> this.putSetting("accurate-drops-with-kill-entire-stack-on-death", true);
            case MUSHROOM_COW -> {
                this.putSetting("drop-additional-mushrooms-for-each-cow-in-stack", true);
                this.putSetting("extra-mushrooms-per-cow-in-stack", 5);
            }
        }
    ...
    public ItemStack getSkullItem() {
        return ItemUtils.getCustomSkull(this.entityTypeData.skullTexture());
    }

    @Override
    public boolean isValidBreedingMaterial(Material material) {
        return this.entityTypeData.breedingMaterials().contains(material);
    }
     ...
     */
    public boolean testCanStackWith(StackedEntityImpl stack1, StackedEntityImpl stack2, boolean comparingForUnstack) {
     ...
     * @return true if the two entities can stack into each other, false otherwise
     */
    public boolean testCanStackWith(StackedEntityImpl stack1, StackedEntityImpl stack2, boolean comparingForUnstack, boolean ignorePositions) {
    ...
    private class StackConditionEntry<T> {
        ...
        public EntityStackComparisonResult apply(EntityStackSettingsImpl stackSettings, StackedEntity stack1,
        ...
        public void setDefaults() {
            ...
            if (configProperties != null)
                EntityStackSettingsImpl.this.setIfNotExists(configProperties.key(), configProperties.defaultValue());
        }

        public void load() {
            StackConditions.ConfigProperties configProperties = this.condition.configProperties();
            if (configProperties != null)
                this.enabled = EntityStackSettingsImpl.this.settingsConfiguration.getBoolean(configProperties.key(), configProperties.defaultValue());
        }
    ...
    public class EntitySetting {
        ...
        public void setDefaults() {
            EntityStackSettingsImpl.this.setIfNotExists(this.key, this.defaultValue);
        }

        public void load() {
            this.value = EntityStackSettingsImpl.this.settingsConfiguration.get(this.key, this.defaultValue);
        }
