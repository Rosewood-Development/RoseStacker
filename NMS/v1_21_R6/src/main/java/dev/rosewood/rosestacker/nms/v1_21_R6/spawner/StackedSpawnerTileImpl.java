package dev.rosewood.rosestacker.nms.v1_21_R6.spawner;
...
public class StackedSpawnerTileImpl extends BaseSpawner implements StackedSpawnerTile {
    ...
    private void trySpawns(boolean onlyCheckConditions) {
        try {
            if (this.nextSpawnData != null) {
                String typeId = this.nextSpawnData.getEntityToSpawn().getStringOr("id", "");
                if (!typeId.isEmpty()) {
    ...
    public void setSpawnerType(SpawnerType spawnerType) {
        ...
        WeightedList.Builder<SpawnData> builder = new WeightedList.Builder<>();
        ...
        this.spawnPotentials.getRandom(RandomSource.create()).ifPresent(x -> this.nextSpawnData = x);
        this.updateTile();
    }
