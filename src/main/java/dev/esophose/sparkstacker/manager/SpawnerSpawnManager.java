...
import dev.esophose.sparkstacker.stack.settings.SpawnerStackSettings.SpawnConditions;
...
public class SpawnerSpawnManager extends Manager implements Runnable {
    ...
    public SpawnerSpawnManager(SparkStacker sparkStacker) {
        super(sparkStacker);
    ...
    public void reload() {
        ...
        this.task = Bukkit.getScheduler().runTaskTimer(this.sparkStacker, this, 0, 1);
    }
    ...
    public void run() {
        ...
        for (Block block : stackManager.getStackedSpawners().keySet()) {
            ...
            SpawnConditions spawnConditions = this.sparkStacker.getStackSettingManager().getSpawnerStackSettings(entityType).getSpawnConditions();
